...
class Speed(Page):
    def __init__(self, parent: Frame, model: Model) -> None:
        ...
        self.gforce_x_label = CTkLabel(
            self.left_frame, text="X: N/A", font=("Lato", 30)
        )
        self.gforce_x_label.grid(row=2, column=0, sticky="nsw", padx=10)

        self.gforce_y_label = CTkLabel(
            self.left_frame, text="Y: N/A", font=("Lato", 30)
        )
        self.gforce_y_label.grid(row=3, column=0, sticky="nsw", padx=10)

        self.gforce_z_label = CTkLabel(
            self.left_frame, text="Z: N/A", font=("Lato", 30)
        )
        ...
        self.spedometer = Spedometer(
            self.spedometer_frame, 0, self.height / 8, self.width / 2, self.height
        )
        ...
        self.current_icon = CTkLabel(
            self.current_frame,
            text="",
            image=BitmapImage(file="images/lightningBolt.xbm", foreground="yellow"),
        )
        self.current_icon.grid(row=0, column=0, sticky="nse")

        self.current_label = CTkLabel(
            self.current_frame, text="N/AA", font=("Lato", 80)
        )
        ...
        self.dcl_icon = CTkLabel(
            self.dcl_frame,
            text="",
            image=BitmapImage(file="images/limitIcon.xbm", foreground="yellow"),
        )
        ...
        self.max_cell_frame.grid_columnconfigure(
            1, weight=1, minsize=self.width * 3 / 16
        )
        self.max_cell_frame.grid(row=2, column=0, sticky="nsew")

        self.max_cell_thermometer = ThermometerProgress(
            self.max_cell_frame, -25, 25, 100, 150, 65, -15
        )
        ...
        self.max_cell_value = CTkLabel(
            self.max_cell_value_frame, text="N/A°", font=("Lato", 50)
        )
        self.max_cell_value.grid(row=0, column=0, sticky="sew")

        self.max_cell_label = CTkLabel(
            self.max_cell_value_frame,
            text="Max Cell",
            font=("Lato", 40),
            wraplength=self.width / 4,
        )
        ...
        self.motor_temp_frame.grid_columnconfigure(
            1, weight=1, minsize=self.width * 3 / 16
        )

        self.motor_temp_thermometer = ThermometerProgress(
            self.motor_temp_frame, -25, 50, 100, 175
        )
        ...
        self.motor_temp_value = CTkLabel(
            self.motor_temp_value_frame, text="N/A°", font=("Lato", 50)
        )
        self.motor_temp_value.grid(row=0, column=0, sticky="sew")

        self.motor_temp_label = CTkLabel(
            self.motor_temp_value_frame, text="Motor", font=("Lato", 40)
        )
    ...
    def update_gforce(self):
        gforce_x = (
            self.model.get_gforce_x()
            if self.model.get_gforce_x() is not None
            else "N/A"
        )
        gforce_y = (
            self.model.get_gforce_y()
            if self.model.get_gforce_y() is not None
            else "N/A"
        )
        gforce_z = (
            self.model.get_gforce_z()
            if self.model.get_gforce_z() is not None
            else "N/A"
        )
        self.gforce_graph.set(gforce_x, gforce_y)
        ...
        if isinstance(gforce_x, float) and isinstance(gforce_y, float):
            self.gforce_label.configure(
                text=f"{round(np.sqrt(np.square(gforce_x) + np.square(gforce_y)))}G"
            )
