...
class Off(Page):
    def __init__(self, parent: Frame, model: Model) -> None:
        ...
        self.off_label = customtkinter.CTkLabel(
            self.middle_frame,
            font=(customtkinter.CTkFont(size=200)),
            text_color="red",
            text="OFF",
        )
        self.off_label.grid(row=0, column=0, sticky="nsew")

        self.precharge_label = customtkinter.CTkLabel(
            self.middle_frame,
            font=(customtkinter.CTkFont(size=50)),
            text_color="red",
            text="N/A",
        )
        ...
        self.pack_temp_thermometer = ThermometerProgress(
            self.pack_temp_frame, 0, 50, 200, 200, 65, -15
        )
        ...
        self.pack_temp_value = customtkinter.CTkLabel(
            self.pack_temp_value_frame, text="N/A°", font=self.label_font
        )
        self.pack_temp_value.grid(row=0, column=0, sticky="sew")

        self.pack_temp_label = customtkinter.CTkLabel(
            self.pack_temp_value_frame, text="Pack", font=self.label_font
        )
        ...
        self.motor_temp_thermometer = ThermometerProgress(
            self.motor_temp_frame, 0, 50, 200, 200
        )
        ...
        self.motor_temp_value = customtkinter.CTkLabel(
            self.motor_temp_value_frame, text="N/A°", font=self.label_font
        )
        self.motor_temp_value.grid(row=0, column=0, sticky="sew")

        self.motor_temp_label = customtkinter.CTkLabel(
            self.motor_temp_value_frame, text="Motor", font=self.label_font
        )
    ...
    def update_pack_temp(self) -> None:
        pack_temp = (
            self.model.get_pack_temp()
            if self.model.get_pack_temp() is not None
            else "N/A"
        )
    ...
    def update_precharge(self) -> None:
        precharge = (
            self.model.get_precharge()
            if self.model.get_precharge() is not None
            else "N/A"
        )
        precharge = self.mapValueToPrecharge(precharge)
        self.precharge_label.configure(
            text=precharge, text_color=precharge_state_color_transformer(precharge)
        )
        if precharge == "Ready":
