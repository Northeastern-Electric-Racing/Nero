...
class Efficiency(Page):
    def __init__(self, parent: Frame, model: Model) -> None:
        ...
        self.temperatures_frame.grid_columnconfigure(
            0, weight=1, minsize=self.width / 4
        )
        self.temperatures_frame.grid_columnconfigure(
            1, weight=1, minsize=self.width / 4
        )
        self.temperatures_frame.grid_columnconfigure(
            2, weight=1, minsize=self.width / 4
        )
        self.temperatures_frame.grid_columnconfigure(
            3, weight=1, minsize=self.width / 4
        )
        ...
        self.accumulator_circular_progress = CircularProgressbar(
            self.accumulator_power_control, 25, 25, 100, 100
        )
        self.accumulator_circular_progress.grid(row=0, column=0, sticky="nsew")

        self.accumulator_power_control_label = CTkLabel(
            self.accumulator_power_control,
            text="Accumulator Max Cooling",
        ...
        self.torque_circular_progress = CircularProgressbar(
            self.torque_power_control, 25, 25, 100, 100
        )
        self.torque_circular_progress.grid(row=0, column=0, sticky="nsew")

        self.torque_power_control_label = CTkLabel(
            self.torque_power_control,
            text="Max Torque",
            font=CTkFont(size=20, weight="bold"),
        ...
        self.regen_circular_progress = CircularProgressbar(
            self.regen_power_control, 25, 25, 100, 100
        )
        self.regen_circular_progress.grid(row=0, column=0, sticky="nsew")

        self.regen_power_control_label = CTkLabel(
            self.regen_power_control,
            text="Max Regen",
            font=CTkFont(size=20, weight="bold"),
            wraplength=self.width / 10,
        )
        ...
        self.motor_circular_progress = CircularProgressbar(
            self.motor_power_control, 25, 25, 100, 100
        )
        self.motor_circular_progress.grid(row=0, column=0, sticky="nsew")

        self.motor_power_control_label = CTkLabel(
            self.motor_power_control,
            text="Motor Max Cooling",
            font=CTkFont(size=20, weight="bold"),
            wraplength=self.width / 10,
        )
        ...
        self.segment_temperature_bar_graph_frame.grid_rowconfigure(
            0, weight=1, minsize=self.height / 8
        )
        ...
        self.segment_temp_ax.bar(
            self.segments, [0, 0, 0, 0], color=["red", "orange", "yellow", "green"]
        )
        self.segment_temp_ax.set_xticklabels(self.segments, color="white")

        self.segment_temp_canvas = FigureCanvasTkAgg(
            self.segment_temp_fig, master=self.segment_temperature_bar_graph_frame
        )
        ...
        self.max_cell_temp_thermometer = ThermometerProgress(
            self.max_cell_temp_frame, 0, 50, 100, 250, high=65, low=-15
        )
        ...
        self.max_cell_temp_value = CTkLabel(
            self.max_cell_value_frame, text="0", font=(CTkFont(size=50))
        )
        self.max_cell_temp_value.grid(row=0, column=0, sticky="sew")

        self.max_cell_temp_label = CTkLabel(
            self.max_cell_value_frame,
            text="Max Cell Temp",
        ...
        self.state_of_charge_battery = BatteryProgress(
            self.state_of_charge_frame, 50, 50, 150, 250
        )
        ...
        self.motor_temp_thermometer = ThermometerProgress(
            self.motor_temp_frame, 0, 50, 100, 125
        )
        ...
        self.motor_temp_value = CTkLabel(
            self.motor_temp_value_frame, text="0", font=(CTkFont(size=50))
        )
        self.motor_temp_value.grid(row=0, column=0, sticky="sew")

        self.motor_temp_label = CTkLabel(
            self.motor_temp_value_frame,
            text="Motor Temp",
            font=(CTkFont(size=25)),
            wraplength=self.width / 8,
        )
        ...
        self.inverter_temp_thermometer = ThermometerProgress(
            self.inverter_temp_frame, 0, 50, 100, 125, 80, -30
        )
        self.inverter_temp_thermometer.grid(row=0, column=0, sticky="nsew")

        self.inverter_temp_value_frame = Frame(
            self.inverter_temp_frame, background="black"
        )
        ...
        self.inverter_temp_value = CTkLabel(
            self.inverter_temp_value_frame, text="0", font=CTkFont(size=50)
        )
        self.inverter_temp_value.grid(row=0, column=0, sticky="sew")

        self.inverter_temp_label = CTkLabel(
            self.inverter_temp_value_frame,
            text="Inverter Temp",
            font=CTkFont(size=25),
            wraplength=self.width / 8,
        )
        ...
        self.ave_temp_canvas = FigureCanvasTkAgg(
            self.ave_temp_fig, master=self.ave_cell_temp_graph_frame
        )
        ...
        self.soc_delta_fig.suptitle(
            "Change in State of Charge", color="white", fontsize=6
        )
        self.soc_delta_ax.set_xlabel("Time [s]", color="white")
        self.soc_delta_ax.tick_params(labelcolor="white")

        # creating the Tkinter canvas containing the Matplotlib figure
        self.soc_delta_canvas = FigureCanvasTkAgg(
            self.soc_delta_fig, master=self.state_of_charge_delta_graph_frame
        )
    ...
    def update_fan_cooling(self):
        percentage = (
            int(self.model.get_fan_power())
            if self.model.get_fan_power() is not None
            else "N/A"
        )
    ...
    def update_torque_power(self):
        percentage = (
            int(self.model.get_torque_power())
            if self.model.get_torque_power() is not None
            else "N/A"
        )
    ...
    def update_regen_power(self):
        percentage = (
            int(self.model.get_regen_power())
            if self.model.get_regen_power() is not None
            else "N/A"
        )
    ...
    def update_motor_power(self):
        percentage = (
            int(self.model.get_motor_power())
            if self.model.get_motor_power() is not None
            else "N/A"
        )
    ...
    def update_pack_segments(self):
        segment1_temp = (
            self.model.get_segment1_temp()
            if self.model.get_segment1_temp() is not None
            else "N/A"
        )
        segment2_temp = (
            self.model.get_segment2_temp()
            if self.model.get_segment2_temp() is not None
            else "N/A"
        )
        segment3_temp = (
            self.model.get_segment3_temp()
            if self.model.get_segment3_temp() is not None
            else "N/A"
        )
        segment4_temp = (
            self.model.get_segment4_temp()
            if self.model.get_segment4_temp() is not None
            else "N/A"
        )
        self.segment_temp_ax.clear()
        self.segment_temp_ax.bar(
            self.segments,
            [segment1_temp, segment2_temp, segment3_temp, segment4_temp],
            color=[
                self.color_transformer(segment1_temp),
                self.color_transformer(segment2_temp),
                self.color_transformer(segment3_temp),
                self.color_transformer(segment4_temp),
            ],
        )
        self.segment_temp_ax.set_xticklabels(self.segments, color="white")
        self.segment_temp_canvas.draw()
    ...
    def update_max_cell_temp(self):
        temp = (
            self.model.get_max_cell_temp()
            if self.model.get_max_cell_temp() is not None
            else "N/A"
        )
    ...
    def update_inverter_temp(self):
        temp = (
            self.model.get_inverter_temp()
            if self.model.get_inverter_temp() is not None
            else "N/A"
        )
